...
class TokoOlahraga:
    ...
    # Jump Search untuk ID atau Nama
    def jump_search(self, key, by_name=False):
        if not self.perlengkapan.head:
            print("Daftar Perlengkapan Olahraga masih kosong.")
            return

        n = self.get_linked_list_length()
        if by_name:
            ...
            if self.perlengkapan.head.data['ID'] == key:
                print("Detail Perlengkapan:")
                print(self.perlengkapan.head.data)
                return
    ...
    def update_perlengkapan(self, nama, jenis, harga, jumlah, ID):
        current = self.perlengkapan.head
        while current:
            if current.data['Nama'] == nama:
                current.data = {'Nama': nama, 'Jenis': jenis, 'Harga': harga, 'Jumlah': jumlah, 'ID': ID}
...
# Setelah pengurutan selesai, tampilkan daftar perlengkapan
    ...
    # Searching Data
    def search(self, by_name=True):
        if by_name:
            nama_search = input("Masukkan Nama perlengkapan yang ingin dicari: ")
            self.jump_search(nama_search, by_name=True)
        else:
            id_search = int(input("Masukkan ID perlengkapan yang ingin dicari: "))
            self.jump_search(id_search)
...
def main():
    ...
    while True:
        ...
        if pilihan == "1":
            ...
            nama_cari = input("Masukkan Nama perlengkapan yang ingin dicari: ")
            toko.cari_perlengkapan(nama_cari)
            input("Tekan Enter untuk kembali ke menu...")
            ...
            jumlah = int(input("Jumlah: "))
            ID = int(input("ID : "))
            toko.update_perlengkapan(nama, jenis, harga, jumlah, ID)
            input("Tekan Enter untuk kembali ke menu...")

        elif pilihan == "5":
            nama_hapus = input("Masukkan Nama perlengkapan yang ingin dihapus: ")
            toko.hapus_perlengkapan(nama_hapus)
            input("Tekan Enter untuk kembali ke menu...")
            ...
            print("\n🎀=+=+=+=+=+=+=PILIH JENIS SEARCHING=+=+=+=+=+=+=+=+=+=🎀")
            print("                                                      +")
            print("[1.] SEARCH BY ID                                      +")                     
            print("[2.] SEARCH BY NAMA                                    +")
            print("                                                      +")
            print("🎀=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=🎀")
            pilihan_search = input("Pilih jenis searching: ")
            if pilihan_search == "1":
                id_search = int(input("Masukkan ID perlengkapan yang ingin dicari: "))
                toko.jump_search(id_search)
                input("Tekan Enter untuk kembali ke menu...")
            elif pilihan_search == "2":
                nama_search = input("Masukkan Nama perlengkapan yang ingin dicari: ")
                toko.jump_search(nama_search, by_name=True)
                input("Tekan Enter untuk kembali ke menu...")
            else:
                print("Pilihan tidak valid.")
